...
服务端返回的 HTML 页面中无网页展示内容`<div id="root"></div>`，然后加载执行JavaScript文件，通过 JavaScript去异步加载数据，渲染生成页面并完成页面交互事件的绑定。
...
* 在服务器端执行一次，用于实现服务器端渲染。使得浏览器下载的 HTML 已经包含了所有的页面展示内容。
...
React中的虚拟DOM本身就是一个内存中的对象，可以轻松的把这个对象转换我们想要的表现形式，比如html格式。我们写的 jsx 看上去就像在写 html（其实写的是对象） 标签，其实经过编译后都会转换成`React.createElement`方法，最终会被转换成一个对象(虚拟DOM)，而且和平台无关。

在服务器端把虚拟 DOM 映射成字符串输出；在客户端将虚拟 DOM 映射成真实 DOM，完成页面挂载。
